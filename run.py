...
MODEL_NAME = 'food_classifier_model.h5'
...
def upload_image():
    ...
    if file and file.filename and allowed_file(file.filename):
        ...
        if not os.path.exists(model_save_at) or MODEL_NAME == '':
            flash('AI model not available. Please contact administrator to set up the trained model.')
        ...
        try:
            ...
            if img is not None:
                ...
                flash('Error reading the uploaded image')
                return redirect(request.url)
